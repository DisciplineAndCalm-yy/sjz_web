...
<script lang="ts" setup>
  ...
  import { uploadUrl, getTemporaryPath } from '/@/api/common/api';
  ...
  const props = defineProps({
     ...
     * 是否返回url，
     * true：仅返回url
     * false：返回storageName storageSize storageId
     */
  ...
  watch(fileList, (val) => nextTick(() => addActionsListener(val)), { immediate: true });

  const antUploadItemCls = 'ant-upload-list-item';

  // Listener
  function addActionsListener(val) {
    console.log('触发了 filelist change', val)
  ...
  function parsePathsValue(paths) {
    ...
    for (const item of paths.split(',')) {
      ...
      list.push({
        ...
        status: 'done',
        url: url,
        response: { status: 'history', result: item },
      });
    }
    fileList.value = list;
  }
  ...
  function parseArrayValue(array) {
    ...
    for (const item of array) {
      console.log('filelistarrray-----', array)
      let unitem = unref(item)
      console.log('unitem----', unitem)
      // let url = getFileAccessHttpUrl(item.filePath);
      let url = unitem.storageId;
      list.push({
        uid: uidGenerator(),
        name: unitem.storageName,
        url: url,
        status: 'done',
        response: { status: 'history', result: {storageId: unitem.storageId} },
      });
    }
    fileList.value = list;
    console.log('设置完之后的filelsit', fileList.value)
  }
  ...
  function onBeforeUpload(file) {
    ...
    console.log('fileeeee--------', file.name,  bindProps.value.accept)
    console.log('propsssssssss-----------', props)
    // 文件类型限制
    if (bindProps.value.accept) {
      let fileExtension = file.name.substring(file.name.lastIndexOf('.') + 1)
      if (!bindProps.value.accept.includes(fileExtension)) {
        createMessage.warning('文件类型错误');
        uploadGoOn.value = false;
        return false
      }
    }
    // 文件大小限制
    if (props.maxSize) {
      if (file.size > bindProps.value.maxSize * 1024 * 1024) {
        createMessage.warning('文件大小超过限制');
        uploadGoOn.value = false;
        return false
      }
    }
  ...
  function onFileChange(info) {
    console.log('onfilechange info', info)
    ...
    if (info.file.status === 'done') {
      ...
      if (info.file.response.success) {
        console.log('要set fiellistmap', fileListTemp)
        fileListTemp = fileListTemp.map((file) => {
          console.log('file-item',file)
          if (file.response) {
            // console.log('url', file.response.result.path)
            let reUrl = file.response.result.storageId;
    ...
    if (info.file.status === 'done' || info.file.status === 'removed') {
      ...
      if (props.returnUrl) {
        ...
        let newFileList: any[] = [];
        console.log('filisttemp change 后----', fileListTemp)
        for (const item of fileListTemp) {
          if (item.status === 'done') {
            let fileJson = {
              storageName: item.name,
              storageId: item.response.result.storageId,
  ...
  function handlePathChange() {
    ...
    let pathList: string[] = [];
    // let storageIdList: string[] = [];
    for (const item of uploadFiles) {
      if (item.status === 'done') {
        pathList.push(item.response.result.path);
        // storageIdList.push(item.response.result.storageId);
      } else {
        return;
      }
    }
  ...
  // 预览文件、图片
  async function onFilePreview(file) {
    console.log('change-file', file)
    let storageId = file.response.result.storageId
    let url = await getTemporaryPath({storageId})
    if (isImageMode.value) {
      createImgPreview({ imageList: [url], maskClosable: true });
    } else {
      // console.log('url--', url, file.name)
      downFile(url, file.name)
    }
  }
  ...
  function getFileName(path) {
    ...
    return path.substring(path.lastIndexOf("/")+1).replace(/_\d+(.[a-zA-Z0-9]+)$/, '$1');
  }
