<template>
    <div class="setContainer">
        ...
        <div class="radioGroup">
            ...
            {{ tableData2 }} -->
            <a-button type="primary" v-if="params.departType == '2' && (tableData2.length < 40)" preIcon="ant-design:plus-outlined"
                style="float: right;" @click="deal(0)">添加</a-button>
            <BasicTable @register="registerTable">
                <template #bodyCell="{ column, record }">
                    <template v-if="column.key === 'sendDepartList' && props.monitorType == '1'">
                        <a-select v-model:value="record.sendDepartList" class="width-100-percent" allowClear
                            show-search optionFilterProp="label" :filter-option="filterOption"
                            :mode="params.departType == '1' ? 'multiple' : undefined"
                            @change="handleChange(record)">
                            <a-select-option :disabled="item.disabled" :label="item.name" :value="item.id" v-for="item in orgs">{{ item.name
                            }}</a-select-option>
                        </a-select>
                    </template>

                    <template
                        v-if="column.key === 'sendDepartList' && (props.monitorType == '2' || props.monitorType == '3')">
                        <!-- <a-cascader v-model:value="record.sendDepartList" :options="orgs" expand-trigger="hover" multiple
                            max-tag-count="2" placeholder="请选择" :fieldNames="fieldNames" :show-checked-strategy="Cascader.SHOW_CHILD"
                            class="width-100-percent" /> -->

                        <a-tree-select v-model:value="record.sendDepartList" style="width: 100%" :tree-data="orgs" show-search 
                            :fieldNames="fieldNames" tree-checkable @change="handleChange(record)" :maxTagCount="4"
                            allow-clear :show-checked-strategy="SHOW_CHILD" placeholder="请选择"
                            tree-node-filter-prop="label" />
                    </template>

                    <template v-if="column.key === 'sendUser'">
                        <a-select v-model:value="record.sendUser" class="width-100-percent" show-search optionFilterProp="label" :filter-option="filterOption" allowClear>
                            <a-select-option :value="item.id" :label="item.name" v-for="item in record.departs">{{ item.name
                            }}</a-select-option>
                        </a-select>
                    </template>
                    <template v-if="column.key === 'checkUser'">
                        <a-select v-model:value="record.checkUser" class="width-100-percent" show-search optionFilterProp="label" :filter-option="filterOption" allowClear>
                            <a-select-option :value="item.id" :label="item.name" v-for="item in checkUsers">{{ item.name }}</a-select-option>
                        </a-select>
                    </template>
                </template>
                <!--操作栏-->
...
const filterNode = (input: string, option: any) => {
...
const fieldNames = reactive({ label: 'name', value: 'id', children: 'children' })
...
const changeDepartType = (a) => {
    console.log('a', a)
    const value = a.target?a.target.value : a;
    // dataSource.value = value == '1' ? tableData1.value : tableData2.value
    setProps({
        showActionColumn: value == 1 ? false : true
    })
    reload()
}
...
const getDepart = (departType, departId, index) => {
    ...
    querySendDepart(params).then(res => {
        ...
        departs.value = result;
        dataSource.value[index].departs = result;
        // if (departType == '2') {
...
const save = () => {
    ...
    const isSame = isSameValue(dataSource.value, 'sendUser')
    if (isSame == true) {
        notification.error({
            message: "警告",
    ...
    //     return item
    // })
    params.departChildren = dataSource.value


    const result = findEmptyValue(params);
    ...
    if (result) {
        ...
        replaySet(params).then(res => {
            emit('getList', { departType: params.departType, questionId: null })
        })
    }
    // return
    // console.log('params', params)
...
//注册table数据
const [registerTable, { reload, clearSelectedRowKeys, setColumns, setProps }, { rowSelection, selectedRows, selectedRowKeys }] = tableContext;
