 ...
 * 平均数组，多余的分数加到第一项
 * @param arr 需要平均的数组
 * @param key 需要平均的数组的key
 * @param [digit=1] 小数点后保留位数,默认为1位
 * @param [full=100] 满分的值,默认为100
 */
export function averageArr(arr, key, digit=1, full=100) {
  if(!key) {
    return arr
  }
  let todigitNum = Math.pow(10, digit)
  let other = Math.floor(full/ arr.length * todigitNum) / todigitNum
  let first = (full * todigitNum - other * todigitNum * ( arr.length -1)) / todigitNum
  // console.log('other', other)
  // console.log('first', first)
  let aveArrs = arr.map((item, index) => {
    if (index == 0) {
      item[key] = first
    } else {
      item[key] = other
