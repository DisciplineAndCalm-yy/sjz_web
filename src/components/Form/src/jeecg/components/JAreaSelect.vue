<template>
  <a-form-item-rest>
    <div class="area-select">
      <!--省份-->
      <a-select v-model:value="province" @change="proChange" allowClear :disabled="disabled" :placeholder="placeholder">
...
export default defineComponent({
  ...
  setup(props, { emit, refs }) {
    ...
    const areaOptions = computed(() => {
      let opts = getDataByCode(pca.city)
      
      if ((pca.city && !opts) || (pca.city && !opts.length)) {
        opts = cityOptions.value.filter(val => val.value === pca.city)
      }
      return pca.city ? opts : [];
    });
    /**
     * 监听props值
     */
    ...
    function initValue() {
      if (props.value) {
        ...
        if (Array.isArray(props.value)) {
          ...
          pca.city = props.value[1] ? props.value[1] : undefined;
          pca.area = props.value[2] ? props.value[2] : undefined;
        } else {
          //传参是数值
          let valueArr = getRealCode(props.value, props.level);
          // 两层的特殊情况
          if (!valueArr.length) {
            valueArr = [...getRealCode(props.value, props.level-1), props.value]
          }
          
          if (valueArr) {
            pca.province = valueArr[0];
            pca.city = props.level >= 2 && valueArr[1] ? valueArr[1] : undefined;
            pca.area = props.level >= 3 && valueArr[2] ? (valueArr[2] || valueArr[1]) : undefined;
          }
        }
      } else {
        //绑定三个数据的情况
        pca.province = props.province ? props.province : undefined;
        pca.city = props.city ? props.city : undefined;
        pca.area = props.area ? props.area : undefined;
      }
    }
    ...
    function proChange(val) {
      ...
      pca.area = pca.city && getDataByCode(pca.city)[0]?.value || pca.city;
    ...
    function cityChange(val) {
      pca.area = val && getDataByCode(val)[0]?.value || val;
